...
function toFolderName(player: string) {
  // Convert hyphens to spaces and handle special cases if needed
...
export default function PlayerPage() {
  ...
  const imageFolder = `/Diamondbacks/${folderName}`;
  ...
  useEffect(() => {
    setLoading(true);
    fetch(
      `/api/diamondbacks-player-images?player=${encodeURIComponent(folderName)}`
    )
      .then((res) => res.json())
      .then((data) => {
